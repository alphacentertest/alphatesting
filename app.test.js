...
describe('POST /login', () => {
  ...
  it('should return 401 for invalid password', async () => {
    const res = await request(app)
      .post('/login')
      .send({ password: 'wrongpassword', _csrf: 'invalid-token' });
    expect(res.statusCode).toEqual(401);
    expect(res.body).toHaveProperty('message', 'Невірний пароль');
  });
});

// Тести для маршруту / (перевірка авторизації)
describe('GET /', () => {
  it('should serve login page for unauthenticated users', async () => {
    const res = await request(app).get('/');
    expect(res.statusCode).toEqual(200);
    expect(res.text).toContain('<h1>Вхід</h1>');
  });
});
